...
get_latest_version() {
    if [[ "$1" == *:*:*:* ]]; then
        # Extract the part after the fourth ':'
...
# A fiery and intense passionate hatred burns in my heart for the LWJGL shitheads and maven kool aid drinkers everywhere.
# How fucking hard is it to just put the binary in a github release? Fuck you, choke on your wife's boyfriend's cock.
get_classifier_suffix() {
    input="$1"
    # Check if the input contains a third ':'
    if [[ "$input" == *:*:* ]]; then
        # Extract the part after the third ':'
        classifier=$(echo "$input" | cut -d: -f3)
        if [[ "$classifier" == "" ]]; then
            echo ""
        else
            echo "-$classifier"
        ...
        # No third ':', return an empty string
        echo ""
    fi
}
...
while IFS= read -r line; do
    ...
    if [[ "$line" =~ ^github: ]]; then
        ...
        suffix=$(get_classifier_suffix "$line")
        jar_url="https://repo1.maven.org/maven2/$group_id/$artifact_id/$latest_version/$artifact_id-$latest_version$suffix.jar"
